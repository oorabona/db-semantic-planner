...
describe('INCLUDE-WHERE-SCOPE-2HOP: 2-hop include WHERE compiled', () => {
	it('where on 2nd-hop include is applied to root query', () => {
		const plan: PlanReport = {
			rootTable: 'calls',
			intent: {
				type: 'select', from: 'calls',
				select: { type: 'fields', fields: ['id'] },
				include: [{
					relation: 'callee', join: 'inner',
					include: [{
						relation: 'file', join: 'inner',
						where: { kind: 'comparison', field: 'project_id', operator: 'eq', value: 42 },
					}],
				}],
			},
			decisions: [
				{ id: 'D1', type: 'include-strategy', choice: 'join', joinType: 'inner',
				  context: { sourceTable: 'calls', target: 'callees', relation: 'callee', relationType: 'belongsTo', intentPath: 'include[0]' },
				  reasoning: 'explicit join:inner', alternatives: [] },
				{ id: 'D2', type: 'include-strategy', choice: 'join', joinType: 'inner',
				  context: { sourceTable: 'callees', target: 'files', relation: 'file', relationType: 'belongsTo', intentPath: 'include[0].include[0]' },
				  reasoning: 'explicit join:inner', alternatives: [] },
			],
			warnings: [], rootTableAlias: undefined, schemaName: undefined,
	...
	it('1-hop include WHERE still works (regression guard)', () => {
		const plan: PlanReport = {
			rootTable: 'symbols',
			intent: {
				type: 'select', from: 'symbols',
				select: { type: 'fields', fields: ['id'] },
				include: [{
					relation: 'file', join: 'inner',
					where: { kind: 'comparison', field: 'project_id', operator: 'eq', value: 7 },
				}],
			},
			decisions: [
				{ id: 'D1', type: 'include-strategy', choice: 'join', joinType: 'inner',
				  context: { sourceTable: 'symbols', target: 'files', relation: 'file', relationType: 'belongsTo', intentPath: 'include[0]' },
				  reasoning: 'explicit join:inner', alternatives: [] },
			],
			warnings: [], rootTableAlias: undefined, schemaName: undefined,
		} as unknown as PlanReport;
	...
	it('2-hop include without where produces no WHERE', () => {
		const plan: PlanReport = {
			...
			intent: {
				type: 'select', from: 'calls',
				select: { type: 'fields', fields: ['id'] },
				include: [{ relation: 'callee', join: 'inner', include: [{ relation: 'file', join: 'inner' }] }],
			},
			decisions: [
				{ id: 'D1', type: 'include-strategy', choice: 'join', joinType: 'inner',
				  context: { sourceTable: 'calls', target: 'callees', relation: 'callee', relationType: 'belongsTo', intentPath: 'include[0]' },
