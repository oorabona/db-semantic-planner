...
describe('resolveLogicalName', () => {
	describe('camelCase convention', () => {
		it('should convert snake_case DB name to camelCase logical name', () => {
			...
			expect(resolveLogicalName(model, 'post_comments', 'camelCase')).toBe(
				'postComments',
			);
		});
		...
		it('should return simple names that match directly', () => {
			...
			expect(resolveLogicalName(model, 'posts', 'camelCase')).toBe('posts');
		});

		it('should return undefined for unknown tables', () => {
			const model = createMockModel(['posts', 'authors']);
			expect(resolveLogicalName(model, 'foo_bar', 'camelCase')).toBeUndefined();
		});

		it('should handle multi-segment snake_case names', () => {
			const model = createMockModel(['userProfileSettings']);
			expect(
				resolveLogicalName(model, 'user_profile_settings', 'camelCase'),
			).toBe('userProfileSettings');
		});
	});

	describe('preserve convention', () => {
	...
	describe('snake_case convention', () => {
		...
		it('should return undefined for unknown tables', () => {
			const model = createMockModel(['posts']);
			expect(
				resolveLogicalName(model, 'unknown', 'snake_case'),
			).toBeUndefined();
		});
	});
	...
	describe('edge cases', () => {
		it('should fallback to exact match if conversion misses', () => {
			...
			expect(resolveLogicalName(model, 'some_table', 'camelCase')).toBe(
				'some_table',
			);
		});
		...
		it('should handle empty model gracefully', () => {
			...
			expect(
				resolveLogicalName(model, 'anything', 'camelCase'),
			).toBeUndefined();
		});

		it('should handle single-word names', () => {
			const model = createMockModel(['users']);
			expect(resolveLogicalName(model, 'users', 'camelCase')).toBe('users');
		});
	});
});
