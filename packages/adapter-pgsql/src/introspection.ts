...
function buildIndexMap(rows: RawIndex[]): Map<string, IndexIR[]> {
	...
	for (const idx of rows) {
		...
		const indexIR: IndexIR = {
			...
			valid: idx.is_valid === true,
			ready: idx.is_ready === true,
...
type LogicalIdentityCarrierShapeRow = {
...
function logicalIdentityCarrierShapeIsManaged(
...
async function logicalIdentityCarrierIsManaged(
	pool: CatalogQueryExecutor,
	qualifiedSideTable: string,
): Promise<boolean> {
	const shape = await pool.query<LogicalIdentityCarrierShapeRow>(
		`SELECT ` +
	...
	if (!logicalIdentityCarrierShapeIsManaged(shape.rows[0])) {
		return false;
	}
	const marker = await pool.query<{ invalid_marker_rows: string }>(
	...
	return marker.rows[0]?.invalid_marker_rows === '0';
}
...
async function queryLogicalIdentityCatalog(
	...
	if (!(await logicalIdentityCarrierIsManaged(pool, qualifiedSideTable))) {
		return [];
	}
