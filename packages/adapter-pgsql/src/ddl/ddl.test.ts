...
describe('Column enhancements in DDL', () => {
	...
	it('should emit COLLATE in column definition', () => {
		const table: import('@dbsp/types').TableIR = {
			name: 'users',
			columns: [{ name: 'name', type: 'string', nullable: false, collation: 'en_US' }],
			foreignKeys: [],
			indexes: [],
		};
	...
	it('should emit GENERATED ALWAYS AS IDENTITY', () => {
		const table: import('@dbsp/types').TableIR = {
			name: 'users',
			columns: [{ name: 'id', type: 'integer', nullable: false, identity: 'always' }],
			foreignKeys: [],
			indexes: [],
		};
		const stmts = generateDDL(makeSchemaWithTable(table));
	...
	it('should emit GENERATED BY DEFAULT AS IDENTITY', () => {
		const table: import('@dbsp/types').TableIR = {
			name: 'users',
			columns: [{ name: 'id', type: 'integer', nullable: false, identity: 'byDefault' }],
			foreignKeys: [],
			indexes: [],
		};
		const stmts = generateDDL(makeSchemaWithTable(table));
	...
	it('should emit COMMENT ON TABLE after indexes', () => {
		...
		expect(stmts[commentIdx]).toMatch(/COMMENT ON TABLE "users" IS 'User accounts'/);
	});

	it('should emit COMMENT ON COLUMN', () => {
		const table: import('@dbsp/types').TableIR = {
			name: 'users',
			columns: [{ name: 'email', type: 'string', nullable: false, comment: 'Primary email' }],
			foreignKeys: [],
			indexes: [],
		};
		const stmts = generateDDL(makeSchemaWithTable(table));
		...
		expect(commentStmt).toMatch(/COMMENT ON COLUMN "users"\."email" IS 'Primary email'/);
	});
