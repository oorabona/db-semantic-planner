...
function createMockPool(
	...
	uniqueColumns: QueryResult<any> = { rows: [] },
	...
	return {
		query: vi
			...
			.mockResolvedValueOnce(uniqueColumns) // single-column unique constraints
...
describe('introspection — standalone sequences', () => {
	it('omits the default bigint data type from pg_sequences', async () => {
		const pool = createMockPool(
			{ rows: [] },
			{ rows: [] },
			{ rows: [] },
		...
		const model = await introspect(pool);
		expect(model.sequences?.get('default_seq')).not.toHaveProperty('dataType');
		expect(model.sequences?.get('narrow_seq')).toMatchObject({
			dataType: 'integer',
			maxValue: '2147483647',
		});
		expect(pool.query.mock.calls[10][0]).toContain(
