...
b | select __proto__`.dump();
			...
			// Testing __proto__ prototype-pollution handling: assert the value under an own __proto__ key, not Object.prototype.
	...
	it('preserves enumerable __proto__ JSON keys without polluting snapshot prototypes', async () => {
		...
		const afterMutation = vi.fn(
			(_ctx, rows: Array<{ profile: Record<string, unknown> }>) => {
				...
				// Testing __proto__ prototype-pollution handling: mutate the own __proto__ key from JSON.parse, not the prototype link.
...
touched | select profile`.all();
		...
		expect(
			// Testing __proto__ prototype-pollution handling: assert the own __proto__ key survives untouched.
		...
		// Testing __proto__ prototype-pollution handling: snapshot must preserve the own __proto__ key's value.
