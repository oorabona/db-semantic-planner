...
describe('[C8] multiline history round-trip', () => {
	...
	it('[C8] multiline query reloads as single entry (regression gate)', () => {
		const multilineQuery = 'from users\nwhere id = 1\nlimit 10';
		history.add(multilineQuery);

		// capturedWrite must not be empty
		expect(capturedWrite).toBeDefined();

		// Simulate reload: create new history that reads what was written
		vi.mocked(fs.existsSync).mockReturnValue(true);
		vi.mocked(fs.readFileSync).mockReturnValue(capturedWrite!);

		const reloaded = new CommandHistory();

		// Must reload as a SINGLE entry, not split into 3 by newlines
		expect(reloaded.length).toBe(1);
		expect(reloaded.getAll()[0]).toBe(multilineQuery);
	});

	it('single-line queries are unaffected by the fix', () => {
		history.add('from users');
		history.add('from posts');
		...
		expect(reloaded.getAll()[0]).toBe('from users');
		expect(reloaded.getAll()[1]).toBe('from posts');
	});
});
