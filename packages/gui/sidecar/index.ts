...
import { type JsonRpcResponse, ErrorCode, decode, encode, error, notification } from "./protocol.js";
...
console.warn = (...args: unknown[]) => stderrWrite(`[WARN] ${args.map(String).join(" ")}`);
...
console.error = (...args: unknown[]) => stderrWrite(`[ERROR] ${args.map(String).join(" ")}`);
...
function send(message: JsonRpcResponse | ReturnType<typeof notification>): void {
...
rl.on("line", async (line: string) => {
	...
	try {
		...
		const code = "code" in (err as object) ? (err as { code: number }).code : ErrorCode.ParseError;
