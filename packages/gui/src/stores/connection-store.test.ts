...
import { useConnectionStore, type ConnectionProfile } from "./connection-store.js";
...
describe("useConnectionStore", () => {
	...
	describe("profiles", () => {
		...
		it("updates a profile", () => {
			useConnectionStore.getState().addProfile(PROFILE);
			useConnectionStore.getState().updateProfile("test-1", { name: "Prod DB" });
		...
		it("does not update non-existent profile", () => {
			useConnectionStore.getState().addProfile(PROFILE);
			useConnectionStore.getState().updateProfile("non-existent", { name: "Nope" });
