...
# Retro-audit 2026-04-21 core follow-ups (L — deferred from PR #52)
...
- [ ] 🔧 [core] Senior L-2 `packages/core/src/planner.ts:1574` `Object.freeze([...intentPaths])` — spread not switched to `.slice()` (other freeze sites were updated in the perf commit; this one was missed). Priority: L (micro-perf)
...
- [ ] 🔧 [core] Senior L-9 — `cursorPaginate({ limit: 0 })` rejected but `.limit(0)` accepted. Inconsistent bound on safe-integer guards. Either harmonize (both reject 0 or both accept) or document the asymmetry. Priority: L (consistency)
- [ ] 🔧 [core] Senior L-10 `packages/core/src/dx/object-filter.ts:121-127` — `isOperatorObject` prototype-walk theoretical attack: a `value = Object.assign(Object.create({$eq: 'x'}), {$gt: 5})` passes `Object.keys` check but `ops.$eq` resolves via prototype. Requires attacker-controlled prototype chain (not reachable via `JSON.parse`). Switch to `Object.hasOwn(ops, '$eq') ? ops.$eq : undefined` for defense-in-depth. Priority: L (theoretical, non-reachable via realistic input)
...
# Bundle 8 follow-ups (L — deferred from PR #53)

> PR #53 merged as `c9fce38` (2026-04-23). 3 SOLID refactors landed across 7 Copilot rounds (R1-R4 + 3 fold-ins). Senior caught 2 M + 3 L pre-Copilot (parallel run with push); Copilot R1 surfaced 3 M + 1 L, R2 surfaced 2 M + 1 L (the rest snapshot-lag), R3 surfaced 1 M + 2 L (pagination-impl proto-pollution + hygiene), R4 surfaced 0 new M + 2 L (hygiene). User bypassed the R3 hard cap once to close R4 cleanly. Items below are explicit deferrals.

- [ ] 🔧 [core] PR #53 R2 perf: `negotiateFeatures` iterates per-checker (15 traversals max) instead of one pass through `model.tables`. Acceptable — runs only once at `createOrm()`, not per query. Consider a single-pass checker bundle if ModelIR size becomes an issue. Priority: L (one-time cost)
- [ ] 🔧 [core] Senior L-3: `QueryBuilderImpl` stores BOTH `this.ctx` and individual `this.model` / `this.strictMode` / `this.adapter` / … fields. Every ctx field is mirrored on `this`. Belt-and-braces but invites "can they diverge?" question — consolidate to ctx-only access OR drop `this.ctx` and reconstruct at `clone()`. Priority: L (consistency)
- [ ] 🔧 [core] PR #53 R4 finding #8: 27 `@internal` tags on widened `QueryBuilderImpl` fields. Not publicly exported (`QueryBuilderImpl` class itself stays internal), so zero Hyrum risk today, but if the class ever leaks via a type alias or deep import it becomes a public contract. Consider a private-symbol-keyed accessor handle for pagination-impl / stream-impl to fully seal the class. Priority: L (defensive future-proofing)
- [ ] 🔧 [core] PR #53 R4 finding #9: `adapter as unknown as import('../adapter.js').Adapter` cast in `createOrmInstance` queryCtx assembly uses a dynamic import form despite `Adapter` being already imported statically. Simplify to `adapter as Adapter<unknown>`. Priority: L (type hygiene)
