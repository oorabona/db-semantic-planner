...
- [ ] 💡 **GUI-025-OOS1** [GUI] Infinite scroll (auto-trigger fetch on scroll near bottom) — Priority: L (from plan checkpoint)
- [ ] 💡 **GUI-025-OOS2** [GUI] Keyset pagination for very large datasets (>100K rows) — Priority: L (from plan checkpoint)
- [ ] 🔧 **GUI-025-F002** [GUI] pageStore (sidecar cursor map) has no TTL or max-entries — memory leak for long sessions — Priority: M (from /review F-002)
