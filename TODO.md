...
- [ ] 🟡 **DDL-COMPLETE** [Adapter] Complete DDL migration system — CHECK, ENUM, SEQUENCE, EXTENSION, PARTITION, index methods/partial/expression, onUpdate FK, collation, comments, identity, multi-schema, deferred constraints, FK auto-index consistency. — Priority: P0
  ...
  - [x] ✅ Block 4: FK enhancements + auto-index — onUpdate/deferred on ForeignKeyIR; introspection query extended (update_rule, is_deferrable, initially_deferred); compareForeignKeys extended; generateAddFKSQL + generateAlterTableAddFK extended; fkAutoIndex in generateMigrationSQL; MigrationSQLOptions.fkAutoIndex; 20 new tests across 3 test files (2026-03-18)
  - [x] ✅ Block 5: Column enhancements — collation/comment/identity on ColumnIR; comment on TableIR; 4 new ChangeKinds (alter_column_collation, alter_column_identity, add_comment, drop_comment); compareComments in schema-diff; phase 15 for comments; changeToUpSQL/Down for all 4 kinds; generateColumnDef COLLATE+GENERATED; generateDDL PASS 4 comments; introspection extended (is_identity, identity_generation, collation_name, pg_description query); 28 new tests across 3 test files (2026-03-18)
  - [x] ✅ Block 6: Sequences + Extensions — SequenceIR on @dbsp/types; extensions/sequences optional on ModelIR + ModelIRImpl; 5 new ChangeKinds (create/drop_extension, create/alter/drop_sequence); compareExtensions/compareSequences in schema-diff; phases at 4/5/8 shared with enum/drop-table/alter-column; changeToUpSQL+Down for all 5 kinds; generateDDL PASS -1/-0.5 for extensions/sequences before tables; introspection steps 8+9 (pg_extension, pg_sequences with SERIAL exclusion via pg_depend deptype='a'); introspection mock updated (9 queries); 42 new tests across 4 test files (2026-03-18)
  - [x] ✅ Block 7: Partitioning (parent only) — PartitionIR on @dbsp/types (RANGE/LIST/HASH, columns[]); TableIR.partition? optional field; PartitionIR re-exported from @dbsp/core; comparePartitions in schema-diff (strategy/columns change → destructive drop_table with isPartitionChange=true); generateCreateTableSQL + generateCreateTable append PARTITION BY clause; introspection step 7b (pg_partitioned_table query, partstrat mapping r/l/h); introspection mock updated (10 queries, 2 toHaveBeenCalledTimes fixed); normalizeTable preserves + normalizes partition columns; 20 new tests across 3 test files (2026-03-18)
  - [x] ✅ Block 8: Multi-schema + parallelization + regression — introspect() 10 sequential queries → single Promise.all; extensions/sequences moved up; qualifyTable verified for all DDL changes; 2385 adapter tests pass, 1977 core, 1111 nql; 0 failures (2026-03-18)
  - [x] ✅ E2E fix: schema-qualify FK REFERENCES in ddl-generator + migration-sql; exclude unique-constraint-backing indexes from introspection; skip FK auto-indexes in compareIndexes; 440 E2E + 2385 unit tests pass (2026-03-18)
  - [x] ✅ Review fixes F-001/F-002/F-003/F-006: generateDownSQL phase 15 (comments); drop_check DOWN $$; generateCreateTableSQL COLLATE+IDENTITY; buildSummary 9 missing ChangeKinds; 2402 tests pass (2026-03-18)
...
(DDL-OPCLASS ramené en scope DDL-COMPLETE Block 3: opclass + WITH params sur IndexIR)
