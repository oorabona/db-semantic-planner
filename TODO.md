...
## ⏭️ OBSOLETE: Double JOIN on flat include with relation.* select (FLAT-BUG-001)

**Status:** Obsolete — scoped to adapter-kysely which was sunset (PGSQL-SUNSET, 2026-01-30).
adapter-pgsql uses a different compiler architecture that doesn't exhibit this issue.
...
## Fixed Bugs (BACKLOG-BUGS, 2026-01-31)

### ✅ CLI REPL: missing `--casing` flag (2026-01-31)
- **Fix:** Added `--casing <snake|camel|none>` flag to REPL command, maps to `dbCasing`

### ✅ Naming convention semantics are inverted (2026-01-31)
- **Fix:** Added `DbCasing` type (`'snake_case' | 'camelCase' | 'preserve'`) with intuitive semantics alongside legacy `NamingConvention`

### ✅ NQL: deep multi-junction traversal (2026-01-31)
- **Fix:** `buildNestedIncludes()` converts dotted relation paths into nested `IncludeIntent[]` tree

### ✅ DDL Generator: composite PK (2026-01-31)
- **Fix:** `primaryKey` optional on `TableIR`, inferred from FK columns for junction tables, DDL generator guards emission
