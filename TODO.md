...
- [x] ✅ [cli/types/gui] SC-8 `LoadedSchema` + `isValidSchema` promoted to `@dbsp/types`; cli and gui sidecar both import from there (PR #61, `b7bb30f`). Added `tableNames` to the guard contract + 7 new tests on the type.
...
- [x] ✅ [mcp-server] `basename` now imported statically from `node:path` (PR #55, `0a1f36b`)
- [x] ✅ [mcp-server] `sanitizeErrorMessage` carries `@stable` JSDoc with public-contract note (placeholders + 500-char cap) (PR #55, `0a1f36b`)
- [x] ✅ [mcp-server] `format-error.ts` literal-`replaceAll` refactor guard comment added (PR #55, `0a1f36b`)
- [x] ✅ [mcp-server] `canonicalRoots` JSDoc corrected to always-non-empty invariant (PR #55, `0a1f36b`)
- [x] ✅ [mcp-server] `schema-loader.test.ts` fixture indent normalized by biome (PR #55, `0a1f36b`)
- [x] ✅ [mcp-server] Ellipsis `'…'` marker documented as public contract in JSDoc (PR #55, `0a1f36b`)
...
- [x] ✅ [core] `createOrmInstance` queryCtx simplified to `adapter as Adapter<unknown>` (dropped dynamic-import cast) (PR #56, `8bc2cef`)
