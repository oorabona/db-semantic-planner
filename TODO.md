...
- [ ] 🔧 [core] Bundle 8 SOLID — `QueryBuilderImpl` god-object (1957 LoC, 40 methods, 30 fields, 12-parameter constructor). Extract `PaginationMixin`, `StreamMixin`, `HookExecutor` to reduce to ~700 LoC. Scoped as follow-up PR. Priority: M (architectural)
- [ ] 🔧 [core] Bundle 8 SOLID — `createOrmInstance` god-factory (494 LoC, 13 positional parameters × 6 call sites). Introduce `QueryBuilderContext` struct to collapse all 6 sites and eliminate silent-reorder footgun. Priority: M (architectural)
- [ ] 🔧 [core] Bundle 8 SOLID — `negotiateFeatures` OCP violation (131 LoC, cognitive complexity 75, 15 hard-coded feature checks). Refactor to `FeatureChecker[]` registry pattern — new features become additive, zero edits to existing code. Priority: M (architectural)
