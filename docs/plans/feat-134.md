...
| Compiler (`compile-expression.ts` + every value-conversion helper) | **ONE shared resolver** `resolveNamedParam(ctx, name)` owning ALL semantics (hasOwn, proto-trio/`__p*` rejection, undefined/NaN/Infinity rejection, structured errors) — consumed by every site that can meet an `NqlNamedParamExpr`: comparison RHS, `expressionToValue()`, `resolveFilterValue()`, `expressionToSql()`, LIKE/coercion paths, mutation assignment compilation, select-expression compilation, CASE branches, JSON function args, range `contains`. NO per-site reimplementation (the FIX-154 dual-derivation lesson). Default branches that would stringify/throw on the new node type are enumerated in B1's test table. Value lands in `WhereComparisonIntent.value` / `ParamExpressionIntent`; `ctx.params` already flows through `compileNestedQuery` |
| `@dbsp/types` | **NONE** — `WhereComparisonIntent.value: unknown` + `ParamExpressionIntent` already fit |
...
- **Architecture decision v2.1 (2026-06-11, operator-approved): the direct compile API bundles provenance so it cannot be dropped.** The v2 sidecar is a separate channel; the public `orm.nql` tag forwards it correctly, but a low-level `nqlCompile(...,{params})` → `adapter.compile()` caller who forgets to forward the sidecar gets object-shaped params reinterpreted as structure. Harden: `nqlCompile()` returns the compiled output as ONE bundle (intent/plan + paramProvenance together) and the adapter NQL-compile entry consumes that bundle, so provenance travels by construction on the direct path too — without reintroducing a marker on the public clean intent (`toIntentIR()` stays clean). Also: the mutation `IN` `{$ref}`→CTE-subquery rewrite must check provenance (a named-param value must never become a subquery / query structure).
- **Architecture decision v2 (2026-06-11, operator-approved): provenance lives OUTSIDE the public intent — no value-mutating marker.** The marker-on-the-value approach (v1 below) contaminated the single intent tree shared by the adapter (internal) and `toIntentIR()`/`plan()`/`dump().plan` (public), producing a recurring bug class (null-bind, CASE-as-structure, public leak, and a lossy public-strip that corrupted SELECT projections by dropping kind/alias). Root fix: the PUBLIC intent is CLEAN by construction — bound scalar values appear as bare values (spec S1: `WhereComparisonIntent.value === 5`), projection expressions keep kind+alias, object-shaped bound values appear as their raw object (never reclassified as structure), NO marker symbol anywhere, NO strip pass. Provenance (which positions are bound params, so the adapter binds even object-shaped values as `$N` and never reinterprets them as structural fieldRef/outerRef) travels to the adapter via a channel that does NOT mutate public intent values — implementation chooses the cleanest of: a separate adapter-facing compiled representation produced directly (not by stripping), or a positional provenance map accompanying the clean intent. Invariants: (1) public surfaces clean per above; (2) adapter binds every param value incl. object-shaped as `$N`; (3) genuine builder-origin structural nodes still compile as structure; (4) no marker on any value; (5) the v1 `intent-provenance.ts` strip pass is removed once unnecessary. This closes the marker class instead of patching its tail.
- **Architecture decision v1 (SUPERSEDED by v2 above): param provenance is marked at the resolver chokepoint, total by construction.** Successive gate findings (null param emitting literal `= null`; simple CASE `WHEN :param` treating a fieldRef-shaped value as a column reference) were symptoms of ONE flaw: param-provenance marking was applied at individual NQL lowering SITES (comparison RHS, then CASE, …) instead of at the single shared resolver. Any site that forgot to mark left its `:param` value to be duck-typed by the adapter as structure (fieldRef/ref) rather than bound — a "params are values, not structure" violation. Root fix: mark provenance INSIDE `resolveNamedParam` (the shared chokepoint) so EVERY resolved `:param` value carries the unforgeable wrapper by construction — no lowering site can omit it; and the adapter's value-vs-structure decision is driven SOLELY by the marker (marked → always bind; unmarked object → structure). Closes the provenance class the way the chokepoint fixes closed the SELECT-expr-reconstruction and allowlist classes.
...
- Then compile succeeds; `WhereComparisonIntent.value === 5`; adapter SQL is `… WHERE users.id = $1` with `params [5]`
