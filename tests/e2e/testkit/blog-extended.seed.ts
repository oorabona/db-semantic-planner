/**
 * Extended Blog Seed Data for Complex Testing
 *
 * Data structure:
...
export async function seedBlogExtendedData(schemaName: string): Promise<void> {
	...
	// Authors
	await sql`
    INSERT INTO ${sql.ref(schemaName)}.authors (id, name, email, active)
    VALUES
  ...
  `.execute(pool);
	...
	await sql`
    INSERT INTO ${sql.ref(schemaName)}.categories (id, name, parent_id)
    VALUES
      (1, 'Technology', NULL),
      (2, 'Web Development', 1),
      (3, 'Databases', 1),
  ...
  `.execute(pool);
	...
	await sql`
    INSERT INTO ${sql.ref(schemaName)}.posts
      (id, title, content, author_id, category_id, published, featured, view_count, created_at)
    VALUES
  ...
  `.execute(pool);
	...
	await sql`
    INSERT INTO ${sql.ref(schemaName)}.comments
      (id, post_id, author_name, content, approved, created_at)
    VALUES
  ...
  `.execute(pool);
	...
	// Tags
	await sql`
    INSERT INTO ${sql.ref(schemaName)}.tags (id, name, slug)
    VALUES
  ...
  `.execute(pool);
	...
	await sql`
    INSERT INTO ${sql.ref(schemaName)}.post_tags (post_id, tag_id)
    VALUES
 ...
 */
export const blogExtendedTestData = {
	authors: {
		total: 3,
		active: 2,
		inactive: 1,
	...
	posts: {
		...
		drafts: 2,
		featured: 2,
		byAuthor: { alice: 4, bob: 3, charlie: 1 },
	},
	comments: {
		total: 15,
		approved: 11,
