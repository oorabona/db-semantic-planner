...
**How it is enforced**, by two guards over different surfaces. `pnpm install --frozen-lockfile` fails when a manifest has drifted from the lockfile — `pg (lockfile: catalog:, manifest: ^8.16.0)` is what it says to #387's shape, though not to every shape (see below). `scripts/check-catalog.mjs` (`pnpm check:catalog`) reads the source `pnpm-workspace.yaml` as the authority on every declared catalog entry, including unused ones, and the lockfile as the authority on resolutions. It then reads every workspace manifest, requiring every present dependency block to be a map and every declaration to use the exact form its dependency's identity calls for: `catalog:` for a third-party package, `workspace:*` or `workspace:^` for a package of this workspace. It also holds the workspace's direct dependencies to one resolved version each. CI runs them in that order, which is load-bearing — the install is what makes the lockfile a faithful description of the manifests.

Source manifests are not what ships, though. `pnpm pack` runs lifecycle scripts, and a `beforePacking` hook rewrites a manifest on its way into the tarball, after every check above has passed. `publish.yml` therefore snapshots the lockfile and every importer manifest after `check-catalog` but before builds; that snapshot, not the mutable post-pack worktree, is the authority from which `scripts/check-packed.mjs` computes each expected packed manifest. Every packed candidate must carry exactly `publishConfig: {"access":"public"}` in both source and tarball — no alternate registry, access mode or other publish override is part of this workflow. For each tarball the guard compares its sole normalised `package/package.json` entry against that source authority and the catalog: every present dependency block must be a map, every `catalog:` declaration must have become exactly the catalog's specifier, `workspace:*` the exact version and `workspace:^` a caret one, no range may still carry a `catalog:` or `workspace:` protocol, and each block's key set must match the source's, so a dependency *added* while packing is caught and not only a rewritten one. Its bounded compatibility envelope drops only `packageManager`, `pnpm`, and `prepublishOnly`/`prepack`/`prepare`/`postpack`/`publish`/`postpublish`; it continues to compare retained `prepublish` and `pack`. Any future normalisation outside that envelope refuses publication. It also refuses an archive with ambiguous normalised `package/package.json` entries, or one carrying `package/node_modules` or `package/npm-shrinkwrap.json` — a bundled copy and a publishable lockfile each put a second version into a consumer's tree without any range saying so. `publish.yml` packs every candidate, verifies all of them, and only then publishes: a bad later tarball must never be discovered after an earlier package is immutable on npm.

**Both guards check for a wrong answer, not for a hostile one, and the difference decides what belongs in them.** The artefact check exists because this repository has already published a wrong manifest: 1.0.0 went out with `workspace:` markers still in it. That is the class it catches — a substitution that did not happen, a dependency that appeared or vanished, a field silently transformed. It is not a defence against a compromised build step or a hostile committer, and it cannot be one: the guards are versioned in the repository they check, and the job that builds also verifies and publishes in one workspace. So a proposed rule earns its place by answering *what accident does this catch?* — refusing a symlink in a tarball our own CI produced answers nothing, while refusing `bundleDependencies` catches a real npm feature added without understanding it. Isolating the publish step from the build is a real improvement and a separate change; overclaiming it here would be worse than the gap.
...
**Beyond the declaration, the check asks what things resolved to, never how they were spelled.** Every `catalog:` declaration must have got the version the catalog itself got — pnpm records that in the lockfile's `catalogs:` section — and an override, a patch, a pnpmfile hook or a resolution mode that moves a project off it shows up as a mismatch without the check knowing which of them did it. Peer contexts are stripped from that identity, but a `patch_hash=` suffix is retained: a patched build is a different store package from the catalog's bare resolution and therefore fails as intended. Identity comes from the same place: pnpm writes an aliased dependency's real package into the resolution, so `pg-old: npm:pg@8.20.0` records `version: "pg@8.20.0"` and reads as a second version of `pg` rather than as an unrelated dependency. Nothing here parses `npm:`, `jsr:`, a git shorthand or an override selector, which is why none of them can be spelled around.

That distinction was learned the hard way: four consecutive review rounds found a different pnpm spelling the guard read wrongly, and each patch admitted the next one. A rule about resolutions has no such surface. Add rules of that kind; do not add another parser.

Two rules divide the identity question between them, and neither is a general proof alone. The resolved name must equal the declaration key, which catches an alias target hiding under an ordinary-looking key. And every catalog entry's own recorded specifier must be a plain range — a whitelist, so a tag, an alias or any source protocol is refused — which covers what a resolved name cannot: `resolvedPackage` manufactures `name: key` for `link:`, `file:` and bare versions, making the name comparison vacuous exactly there.

Which projects form the workspace comes from pnpm — `pnpm list -r --depth -1 --json` — and must equal the lockfile's `importers` **as a set**, in both directions. Reimplementing the discovery would mean reproducing YAML escape semantics and pnpm's glob handling, copies that drift from the original; but trusting `importers` alone is not enough either, because `sharedWorkspaceLockfile: false` reduces the root lockfile to `{".": {}}` and every package manifest would go unread while the check reported success. That refusal also rejects filtered and `--ignore-workspace` installs, which is correct: this guard has one shared resolution graph to inspect. One bound remains, and is stated in the guard: a repo-level setting that narrows both pnpm's discovery and the install leaves the two views agreeing.

**And it refuses inputs it cannot reason about, rather than certifying around them.** A lockfile that is not the one pnpm installed from — `lockfile: false`, `gitBranchLockfile: true`, a stale checkout — is caught by comparing it with `node_modules/.pnpm/lock.yaml`. A pnpmfile is refused on the `pnpmfileChecksum` pnpm records, which is written whatever the hook file is called or wherever `pnpmfile:` points it; absence is detection rather than proof, since a hook can appear after the install or run only while packing, and CI's frozen install immediately before the check is what makes it strong there. `bundleDependencies` and `bundledDependencies` are refused in any manifest, both spellings, because a bundled copy ships inside the tarball where no range comparison can see it. A project whose effective manifest is `package.json5` stops the check, since it cannot parse one; a `package.json5` that pnpm would not read, beside a `package.json`, is inert and accepted.
