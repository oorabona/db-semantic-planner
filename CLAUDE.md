...
Source manifests are not what ships, though. `pnpm pack` runs lifecycle scripts, and a `beforePacking` hook rewrites a manifest on its way into the tarball, after every check above has passed. So `scripts/check-packed.mjs` reads the finished artefact: for each tarball it compares `package/package.json` against the source manifest and the catalog — every present dependency block must be a map, every `catalog:` declaration must have become exactly the catalog's specifier, `workspace:*` the exact version and `workspace:^` a caret one, no range may still carry a `catalog:` or `workspace:` protocol, and each block's key set must match the source's, so a dependency *added* while packing is caught and not only a rewritten one. Its bounded compatibility envelope drops only `packageManager`, `pnpm`, and `prepack`/`prepare`/`postpack`; `publishConfig` may carry only `access` and `registry`, because a value override would require predicting pnpm rather than checking it. Any future normalisation outside that envelope refuses publication. It also refuses an archive carrying `package/node_modules` or `package/npm-shrinkwrap.json` — a bundled copy and a publishable lockfile each put a second version into a consumer's tree without any range saying so. `publish.yml` packs every candidate, verifies all of them, and only then publishes: a bad later tarball must never be discovered after an earlier package is immutable on npm.
