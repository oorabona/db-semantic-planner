...
**Beyond the declaration, the check asks what things resolved to, never how they were spelled.** Every `catalog:` declaration must have got the version the catalog itself got — pnpm records that in the lockfile's `catalogs:` section — and an override, a patch, a pnpmfile hook or a resolution mode that moves a project off it shows up as a mismatch without the check knowing which of them did it. Identity comes from the same place: pnpm writes an aliased dependency's real package into the resolution, so `pg-old: npm:pg@8.20.0` records `version: "pg@8.20.0"` and reads as a second version of `pg` rather than as an unrelated dependency. Nothing here parses `npm:`, `jsr:`, a git shorthand or a `parent>child` override selector, which is why none of them can be spelled around.

That distinction was learned the hard way: three consecutive review rounds found a different pnpm spelling the guard read wrongly, and each patch admitted the next one. A rule about resolutions has no such surface. Add rules of that kind; do not add another parser.

The check does not decide which projects form the workspace either — it takes the list from the lockfile's `importers`, where pnpm records the projects it discovered. Reimplementing that would mean reproducing YAML escape semantics, pnpm's glob handling and its manifest formats, copies that drift from the original.

**And it refuses inputs it cannot reason about, rather than certifying around them.** A lockfile that is not the one pnpm installed from — `lockfile: false`, `gitBranchLockfile: true`, a stale checkout — is caught by comparing it with `node_modules/.pnpm/lock.yaml`. A `.pnpmfile.cjs` is refused outright: `hooks.beforePacking` rewrites the manifest on its way into the tarball, after every check here has passed. A project whose effective manifest is `package.json5` stops the check, since it cannot parse one; a `package.json5` that pnpm would not read, beside a `package.json`, is inert and accepted.
