...
**The accepted forms are a closed set, and every widening is a way back to two versions.** `catalog:legacy` can name a second range for a package the default catalog already names, so named catalogs are refused outright rather than at each reference. An alias entry (`pg-old: npm:pg@8.20.0`) is a second name for one package, which every later check would read as two unrelated dependencies, so the catalog may not contain one. `workspace:~` publishes a different contract from `*` and `^`. Needing any of these is a deliberate decision that starts by editing the guard.

The one question the check does not answer for itself is which projects form the workspace: it takes that list from the lockfile's `importers` section, where pnpm records the projects it discovered. Reimplementing that discovery would mean reproducing YAML escape semantics, pnpm's glob handling and its manifest formats — copies that drift from the original.
